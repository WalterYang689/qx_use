...
if (process.env.ALPHA_COOKIE) {
    if (process.env.ALPHA_COOKIE.indexOf('&') > -1) {
        CookieAlphas = process.env.ALPHA_COOKIE.split('&');
    } else if (process.env.ALPHA_COOKIE.indexOf('\n') > -1) {
        CookieAlphas = process.env.ALPHA_COOKIE.split('\n');
    } else {
        CookieAlphas = [process.env.ALPHA_COOKIE];
    }
}
...
function startNewRoundWork(user, alphack) {
    return new Promise((resolve) => {
        axios({
            method: "post",
            ...
            headers: {
                "Cookie": alphack,
                "User-Agent": USER_AGENTS[randomNumber(0, USER_AGENTS.length)]
            },
            data: {
    ...
    return new Promise((resolve) => {
        ...
        $.get(url, async (err, resp, data) => {
            try {
                ...
                if (result.Succeeded) {
                    await getCurUserInfo(alphack, result.RewardText)
                } else {
                    await getCurUserInfo(alphack, '时间未到,领取失败!')
                }
            } catch (e) {
                $.logErr(e, resp);
            } finally {
